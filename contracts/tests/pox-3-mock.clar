...
(define-public (unlock-mock (account principal))
    (let (
        (locked-amount (get locked (stx-account-mock account)))
    )
...
;;-------------------------------------
;; PoX 3 
;; https://explorer.hiro.so/txid/0xf9e3ab172a3fd18da7a451b67f9ae686af04b8f520586394c597613099efc83e?chain=mainnet 
;;-------------------------------------
    ...
    (let ((first-reward-cycle (+ u1 (current-pox-reward-cycle)))
      ...
      (let ((reward-set-indexes (try! (add-pox-addr-to-reward-cycles pox-addr first-reward-cycle lock-period amount-ustx tx-sender))))
         ...
         (map-set stacking-state
          ...
          ;; MOCK - Transfer STX to this contract and update map
          (unwrap! (stx-transfer? amount-ustx tx-sender (as-contract tx-sender)) (err 80001))
          (map-set stx-account-map { account: tx-sender } (merge (stx-account-mock tx-sender) { 
            locked: amount-ustx, 
            unlock-height: (reward-cycle-to-burn-height (+ first-reward-cycle lock-period)),
          }))
...
(define-public (stack-increase (increase-by uint))
   (let ((stacker-info (stx-account-mock tx-sender))
      ...
      (asserts! (is-some (fold increase-reward-cycle-entry
        ...
        ;; MOCK - Update locked
        (unwrap! (stx-transfer? increase-by tx-sender (as-contract tx-sender)) (err 80001))
        (map-set stx-account-map { account: tx-sender } (merge (stx-account-mock tx-sender) {
